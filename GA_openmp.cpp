...
char* target = "Hello World!Hello World!Hello World!Hello World!Hello World!Hello World!Hello World!Hello World!Hello World!Hello World!";
...
void mate(offspring* parent, offspring* child){
 ...
 //unsigned int seed = time(NULL);
	...
	for (num = 0; num< CHILD_NUM_PER_GENERATION;num+=4){
		...
		//printf("%d\n",child_num);
		//srand(time(NULL));
		unsigned int seed = clock()+num;
		int parent1 = rand_r(&seed)%CHILD_NUM_PER_GENERATION + 1;
		/*choose parent 2 with the number of not chosen numbers*/
		int parent2 = rand_r(&seed)%CHILD_NUM_PER_GENERATION + 1;
		...
		int parent3 = rand_r(&seed)%CHILD_NUM_PER_GENERATION + 1;
		int parent4 = rand_r(&seed)%CHILD_NUM_PER_GENERATION + 1;
		...
		int i = rand_r(&seed)%(STR_LENGTH+1);/*here i will be used as a separator of how we are going to do crossover with the parents */
		for(j=0;j<i;j++){
